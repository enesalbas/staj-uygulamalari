...
if (!orgAdi) {
  console.error("Kullanim: npm run sync -- <organizasyon-adi>");
  process.exit(1);
}

const BASE_URL = "https://api.github.com";

const sqlite = new Database("repos.db");
...
function repoyaDonustur(hamRepo: any) {
  return {
    id: hamRepo.id as number,
    name: hamRepo.name as string,
    language: (hamRepo.language ?? null) as string | null,
    stars: hamRepo.stargazers_count as number,
    url: hamRepo.html_url as string,
...
async function main() {
  ...
  const donusturulmus = hamRepolar.map(repoyaDonustur);
  kaydet(donusturulmus);
  console.log("Veritabanina kaydedildi (upsert).");
